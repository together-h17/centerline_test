...
# output_combined_path = os.path.join(root, 'combined_output_river.shp')
output_line_shapefile = os.path.join(root, 'output_line_river.shp')
...
for idx, node in tqdm(filtered_gdf.iterrows(), total=filtered_gdf.shape[0]):
    ...
    crossover_count = 0
    for geom in crossover:
        if not geom.is_empty:
            geometries = geom.geoms if geom.geom_type == 'MultiPoint' else [geom]
            crossover_count += len(geometries)
...
for roadsegid, group in grouped:
    ...
    if len(group) > 1:
        ...
        line_attributes = first_point_attributes.to_frame().T # 繼承第一個點屬性
